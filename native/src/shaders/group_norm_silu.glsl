...
void main(){
 ...
 float sum=0,square=0;for(uint i=lane;i<count;i+=256){float v=values.data[base+i];sum+=v;square+=v*v;}
 ...
 for(uint i=lane;i<count;i+=256){uint channel=group*channels_per_group+i/p.spatial;
  float v=(values.data[base+i]-mean)*inverse*scale.data[channel]+bias.data[channel];
  values.data[base+i]=v/(1.0+exp(-v));}
}
