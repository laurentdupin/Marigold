...
int main() try {
    const char* model_environment = std::getenv("MARIGOLD_MODEL");
    ...
    if ((capabilities.flags & required) != required)
        throw std::runtime_error("Marigold ABI2 GPU capability is incomplete");
    ...
    const char* prompt_environment = std::getenv("MARIGOLD_PROMPT_CACHE");
    if (!prompt_environment || !std::filesystem::exists(prompt_environment))
        return 77;
    const char* vae_environment = std::getenv("MARIGOLD_VAE_MODEL");
    ...
    constexpr std::uint32_t width = 768u, height = 64u;
    ...
    check_counter(marigold_get_transfer_counters(
        &upload_before, &download_before), "transfer counters before");
    ...
    if (submit_ms > 5.0) throw std::runtime_error("Marigold submit exceeded 5 ms");
    ibrh_synchronization ready = target.synchronization;
    ibrh_job_status status{};
    for (unsigned attempt = 0; attempt < 60000; ++attempt) {
    ...
    if (status.state != IBRH_JOB_COMPLETE || status.source_frame_id != 7007u)
        throw std::runtime_error("Marigold ABI2 job did not complete with correlation");
    wait_fence(selected.device.Get(), ready);
    std::uint64_t upload_after = 0u, download_after = 0u;
    check_counter(marigold_get_transfer_counters(
        &upload_after, &download_after), "transfer counters after");
    if (upload_after != upload_before || download_after != download_before)
        throw std::runtime_error(
            "Marigold external path staged tensor bytes through host");
    ...
    if (finite_count < gpu.size() / 2u)
        throw std::runtime_error("Marigold external output has too few valid mask pixels");
    ...
    for (std::uint64_t frame = 7008u;
        ...
        if (status.state != IBRH_JOB_COMPLETE || status.source_frame_id != frame)
            throw std::runtime_error("Marigold stress correlation failed");
        wait_fence(selected.device.Get(), ready);
    }
    api.job_release(job);
